...
// simd can't use a real serde deserializer and RawValue technique because simd_json doesn't have a RawValue like that ... wellll...
// if we accept that and do a custom function, we COULD do something similar - but that's a bit time consuming if you support all variants
// but with simd we CAN attempt to look for "marker required properties" in the incoming JSON 'Tape' - what characteristics identify them... 

pub fn deserialize(b: &mut [u8]) -> Result<LambdaRequest,JsonError> {
    let d = aws_lambda_json_impl::JsonDeserializer::from_slice(b)?;
    let t = d.into_tape();
    let v = t.as_value();
    ...
    if let (Some(rc), true) = (v.get_object("request_context"),v.contains_key("http_method")) {
        if rc.get("http").is_none() {
            let res: ApiGatewayProxyRequest = t.deserialize()?;
    ...
    if let Some(rc) = v.get_object("request_context") {
        if rc.get("http").is_some() {
            let res: ApiGatewayV2httpRequest = t.deserialize()?;
    ...
    if let Some(rc) = v.get_object("request_context") {
        if let Some(_) = rc.get("elb") {
            let res: AlbTargetGroupRequest = t.deserialize()?;
            return Ok(LambdaRequest::Alb(res));
        }
    }
    #[cfg(feature = "apigw_websockets")]
    if v.contains_key("connected_at") {
        let res: ApiGatewayWebsocketProxyRequest = t.deserialize()?;
...
mod tests {
    ...
    fn test_deserialize_apigw_rest() {
        ...
        let req: LambdaRequest = deserialize(data.as_mut_slice()).expect("failed to deserialize apigw rest data");
        match req {
    ...
    fn test_deserialize_apigw_http() {
        ...
        let req: LambdaRequest = deserialize(data.as_mut_slice()).expect("failed to deserialize apigw http data");
        match req {
    ...
    fn test_deserialize_sam_rest() {
        ...
        let req: LambdaRequest = deserialize(data.as_mut_slice()).expect("failed to deserialize SAM rest data");
        match req {
            LambdaRequest::ApiGatewayV1(req) => {
                assert_eq!("123456789012", req.request_context.account_id.unwrap());
            }
    ...
    fn test_deserialize_sam_http() {
        ...
        let req: LambdaRequest = deserialize(data.as_mut_slice()).expect("failed to deserialize SAM http data");
        match req {
            LambdaRequest::ApiGatewayV2(req) => {
                assert_eq!("123456789012", req.request_context.account_id.unwrap());
            }
    ...
    fn test_deserialize_alb() {
        ...
        let req: LambdaRequest = deserialize(data.as_mut_slice()).expect("failed to deserialize alb rest data");
        match req {
    ...
    fn test_deserialize_apigw_websocket() {
        ...
        let req: LambdaRequest = deserialize(data.as_mut_slice()).expect("failed to deserialize apigw websocket data");
        match req {
